...
export function effectiveMs(time: Time): number | null {
    switch (time.modifier) {
        case Modifier.DNF:     return null
        case Modifier.PlusTwo: return time.timestamp_in_millis + 2000
        default:               return time.timestamp_in_millis
    }
}
...
export function formatDate(unix_ms: number): string {
    if (!unix_ms) return "—"
    return new Date(unix_ms).toLocaleDateString("en", { month: "short", day: "numeric" })
}
