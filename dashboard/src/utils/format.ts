...
export function computeStdDev(times: Time[]): number | null {
    ...
    const variance = effective.reduce((sum, ms) => sum + Math.pow(ms - mean, 2), 0) / effective.length;
    return Math.round(Math.sqrt(variance));
}

export function computeCountByModifier(times: Time[], modifier: Modifier): number {
...
export function computePercentile(times: Time[]): number | null {
    ...
    const valid = times.map(effectiveMs).filter((ms): ms is number => ms !== null);
    const slower = valid.filter((ms) => ms > lastMs).length;
    return Math.round((slower / valid.length) * 100);
}

export function computeDistribution(times: Time[], bucketCount = 12): DistributionBucket[] {
    const effective = times.map(effectiveMs).filter((ms): ms is number => ms !== null);
