...
function ModalStatCard({ label, value }: { label: string; value: string }) {
    ...
    return (
        <div className="bg-raised rounded-lg px-3 py-2.5 border border-border flex flex-col gap-1">
            ...
            <span className={`font-mono text-sm font-semibold tabular-nums ${dim ? "text-text-muted" : "text-text"}`}>{value}</span>
        </div>
    );
}
...
function TimeDetailsModal({ time, solveIndex, isBest, bestMs, onClose }: TimeDetailsModalProps) {
    ...
    return (
        <button
            type="button"
            className="fixed inset-0 z-50 bg-black/55 backdrop-blur-sm flex items-center justify-center px-4 cursor-default"
            onClick={onClose}
            onKeyDown={(e) => {
