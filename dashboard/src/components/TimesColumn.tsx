...
function ModalStatCard({label, value}: {label: string; value: string}) {
    ...
    return (
        <div className="bg-cell border border-border px-3 py-2.5 flex flex-col gap-1">
            ...
            <span className={`font-mono text-sm font-semibold tabular-nums ${dim ? "text-muted" : "text-ink"}`}>{value}</span>
        </div>
    );
}
        ...
        >
            ...
            >
                <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                    <div>
                        <p className="text-[10px] uppercase tracking-[0.12em] text-muted font-semibold">Solve {solveIndex}</p>
                        <h3 className="text-lg font-mono font-semibold text-ink mt-1">{formatTime(time)}</h3>
                    </div>
                    <button
                        type="button"
                        onClick={onClose}
                ...
                <div className="px-5 pb-5">
                    <p className="text-[10px] uppercase tracking-[0.12em] text-muted font-semibold mb-1.5">Solved At</p>
                    <p className="text-sm text-muted mb-4">{formatDateTime(time.solved_at_unix_ms)}</p>

                    <p className="text-[10px] uppercase tracking-[0.12em] text-muted font-semibold mb-1.5">Scramble</p>
    ...
    return (
        <>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5">
                    ...
                    <section className="h-full border border-border bg-surface p-5 sm:p-6 flex flex-col justify-between animate-fade-in-up">
                        <div>
                            <div className="flex items-center gap-2 mb-4">
                                <span className="text-xs font-semibold text-accent uppercase tracking-[0.08em]">{eventLabel}</span>
                                <span className="text-xs text-muted">·</span>
                                <span className="text-xs text-muted font-mono tabular-nums">{times.length} solve{times.length !== 1 ? "s" : ""}</span>
                            </div>
                ...
                {/* History */}
                <div className="lg:col-span-12">
                    <HistoryModule times={times} bestMs={bestMs} onOpenSolve={handleOpen} />
                </div>
            </div>
