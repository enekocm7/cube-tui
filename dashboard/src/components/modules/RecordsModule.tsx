...
function RecordCard({label, value}: RecordCardProps) {
    ...
    return (
        <div className="bg-cell border border-border px-3 py-2.5 flex flex-col gap-0.5">
            ...
            <span className={`font-mono text-sm font-medium tabular-nums ${dim ? "text-muted" : "text-ink"}`}>
                {value}
            </span>
        </div>
    );
...
export function RecordsModule({bestSingle, bestAo5, bestAo12, bestAo50, bestAo100}: RecordsModuleProps) {
    return (
        <section className="h-full border border-border bg-surface animate-fade-in-up p-4 sm:p-5 flex flex-col">
            <div className="flex items-baseline justify-between mb-4">
                <h2 className="text-sm font-medium text-ink tracking-tight">Session records</h2>
            </div>
