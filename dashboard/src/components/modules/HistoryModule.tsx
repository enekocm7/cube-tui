    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 px-4 sm:px-5 py-2.5 h-11">
                ...
                <span className={`font-mono text-sm font-medium tabular-nums w-32 sm:w-36 shrink-0 ${timeColor}`}>
                    {formatTime(time)}
                    {isBest && (
...
export function HistoryModule({ times, bestMs, onOpenSolve }: HistoryModuleProps) {
    ...
    return (
        <section className="border border-border bg-surface overflow-hidden animate-fade-in-up">
            ...
            {times.length === 0 ? (
                <div className="py-20 text-center text-muted text-sm">No solves recorded.</div>
            ) : (
