...
function StatCard({ label, value }: StatCardProps) {
	...
	return (
		<div className="bg-cell border border-border px-3 sm:px-4 py-3 flex flex-col gap-1 min-w-0">
			...
			<span
				className={`font-mono text-base sm:text-lg leading-tight font-medium tabular-nums whitespace-nowrap ${dim ? "text-muted" : "text-ink"}`}
			>
				{value}
			</span>
		</div>
	...
	return (
		<section className="h-full border border-border bg-surface animate-fade-in-up p-4 sm:p-5">
			...
			<p className="mt-3 text-[10px] text-muted">
				Event: <span className="text-ink">{eventLabel}</span>
			</p>
		</section>
	);
}
